...
public class worstFit {
    public worstFit(ArrayList<Partition> partitions, ArrayList<Process> processes) {
        ...
        for (Process pro : pros) {
            Partition worst = new Partition(0,-1);
            for (Partition par : pars) {

                if (par.totalSize >= pro.siz && !par.fragmented && par.totalSize > worst.totalSize) {
                    worst = par;
                }
            ...
            if (worst.index != -1){
                if (worst.totalSize > pro.siz){
                    int rest = worst.totalSize - pro.siz;
                    worst.totalSize = pro.siz;
                    worst.fragmented = true;
                    worst.ps = pro ;
                    Partition p = new Partition(rest , done);
                    pars.add(worst.index+1 , p);
                    done++;
                    pro.exu = true;
                }
                else if (worst.totalSize == pro.siz){
                    worst.fragmented = true;
                    worst.ps = pro ;
                    pro.exu = true;
                }
